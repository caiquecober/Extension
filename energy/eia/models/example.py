...
class EIAQueryParams(QueryParams):
    series: str = Field(description="EIA series identifier.")
...
class EIAFetcher(Fetcher[EIAQueryParams, List[EIATimeSeriesData]]):
    ...
    @staticmethod
    def extract_data(query: EIAQueryParams, credentials: Optional[Dict[str, str]], **kwargs: Any) -> List[dict]:
        api_key = credentials.get("eia_api_key") if credentials else ""
        response = get_ts(query.series, api_key)  # Assuming get_ts is modified to accept api_key
        return response.to_dict(orient='records')

    @staticmethod
    def transform_data(query: EIAQueryParams, data: List[dict], **kwargs: Any) -> List[EIATimeSeriesData]:
        return [EIATimeSeriesData(date=d['period'], value=d['value']) for d in data]
